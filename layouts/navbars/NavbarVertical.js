...
// Role-based menu filtering function
const roleBasedMenu = (roleId) => {
	if (roleId === '2') {
		return allMenuItems.filter((item) => item.title === 'Dashboard' || item.title === 'Questions' || item.title === "Role Management");
	}
	else {
		return allMenuItems.filter((item) => item.title !== 'Questions' && item.title !== "Role Management");
	}
};
...
const useMenuItems = () => {
	...
	const updateMenuItems = () => {
		...
		setMenuItems(roleBasedMenu(currentRoleId));
	};
			...
			<SimpleBar style={{ maxHeight: '100vh' }}>
				<div className="nav-scroller">
					<Link href="/" className="navbar-brand">
						...
						<h2 className='text-light'>AStarPrep</h2>
					</Link>
				</div>
				{/* Dashboard Menu */}
